    ...
    <main className={styles.page}>
      ...
      <section className={styles.hero}>
        ...
        <div className={styles.heroCopy}>
          ...
          <p className={styles.lead}>PAWU 병원 프로그램의 최신 버전을 내려받고 설치 방법과 업데이트 내용을 한곳에서 확인하세요.</p>
          <div className={styles.badges}><span>Windows 10/11</span><span>64-bit</span><span>병원용</span></div>
        </div>

        <div className={styles.downloadCard}>
          <div className={styles.cardHead}>
            <div><small>CURRENT VERSION</small><strong>{installerVersion}</strong></div>
      ...
      <section className={styles.section}>
        <div className={styles.sectionHeading}><p>INSTALL GUIDE</p><h2>설치는 3단계면 끝납니다.</h2></div>
      ...
      <section className={`${styles.section} ${styles.updateSection}`}>
        <div className={styles.sectionHeading}><p>UPDATE</p><h2>최근 업데이트</h2></div>
        <div className={styles.updateCard}>
          <div className={styles.versionBlock}><small>LATEST</small><strong>{installerVersion}</strong></div>
          <ul>{releaseNotes.slice(0, 8).map((note) => <li key={note}>{note}</li>)}</ul>
        </div>
      </section>

      {release?.sha256 && (
        <section className={styles.section}>
          <div className={styles.sectionHeading}><p>FILE CHECK</p><h2>공식 설치파일 확인</h2></div>
          <div className={styles.updateCard}>
            <div className={styles.versionBlock}><small>SHA256</small><strong>검증값</strong></div>
            <code style={{ overflowWrap: "anywhere", fontSize: 12 }}>{release.sha256}</code>
          </div>
        </section>
      )}
